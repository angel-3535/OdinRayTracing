...
build_font_atlas :: proc() -> mem.Allocator_Error {
	...
	font_texture := cast(^rl.Texture2D)cast(uintptr)io.Fonts.TexID
	if font_texture != nil && font_texture.id != 0 {
		rl.UnloadTexture(font_texture^)
		mem.free(font_texture)
	}

	font_texture = cast(^rl.Texture2D)(mem.alloc(
			size_of(rl.Texture2D),
			align_of(rl.Texture2D),
		) or_return)
	font_texture^ = rl.LoadTextureFromImage(image)
	rl.UnloadImage(image)
	io.Fonts.TexID = cast(imgui.TextureID)cast(uintptr)font_texture
...
render_triangles :: proc(
	...
	texture := cast(^rl.Texture)cast(uintptr)texture_ptr
...
setup_backend :: proc() {
	...
	platform_io.Platform_SetClipboardTextFn = set_clip_text_callback
	platform_io.Platform_GetClipboardTextFn = get_clip_text_callback
	platform_io.Platform_ClipboardUserData = nil
}
